...
int main(int argc, char * argv[]){
  ...
  viewdrift->SetArea(-3.0, 0.0, -0.1, 3.0, Htot, 0.1 );
  ...
  const double energy = 4.e+09; // eV/c
  ...
  double track_x = 2.9;// [cm]
  double track_y = 0.3;
  double track_z = 0.0;
  
   // Momentum direction of incoming track
  double track_dx = -0.9;
  double track_dy = -0.1; // Track coming downstream
  double track_dz = 0.0;
  ...
  track->NewTrack(xcls, ycls, zcls, tcls, track_dx, track_dy, track_dz);
